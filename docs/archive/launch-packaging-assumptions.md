...
**Status:** Draft v0.1  
**Intent:** Capture packaging logic for 50 kW modules. Numbers here are placeholders until
radiator/array are locked.
...
We do not lock exact launchers yet. We assume common cylindrical fairing envelopes and build the
module to be:
...
- battery for eclipse (if applicable), we only define the _method_:
...
- At **1200 K**, radiator area is far smaller than conventional low-T radiators, so **arrays and
  power** likely dominate stowed volume.
