...
exports.emitMarkdownSummary = emitMarkdownSummary;
exports.emitFlagReport = emitFlagReport;
exports.formatAssumption = formatAssumption;
// ─── Display unit helpers §10.3 ───────────────────────────────────────────────
function fmtW(w) {
    if (Math.abs(w) >= 1e6)
        return `${(w / 1e6).toFixed(3)} MW`;
    if (Math.abs(w) >= 1e3)
        return `${(w / 1e3).toFixed(2)} kW`;
 ...
 */
function emitMarkdownSummary(result) {
    ...
    lines.push(``);
    if (t.t_zone_a_k !== null)
        lines.push(`- Zone A: ${fmtK(t.t_zone_a_k)}`);
    if (t.t_zone_b_k !== null)
        lines.push(`- Zone B: ${fmtK(t.t_zone_b_k)}`);
    if (t.t_zone_c_k !== null)
        lines.push(`- Zone C: ${fmtK(t.t_zone_c_k)}`);
    if (t.t_zone_d_k !== null)
        lines.push(`- Zone D: ${fmtK(t.t_zone_d_k)}`);
    lines.push(``);
    ...
    lines.push(`## Flags`);
    lines.push(``);
    lines.push(`Errors: ${result.flag_summary.error_count} | ` +
    ...
    if (hasFlags) {
        for (const flag of result.flags) {
            lines.push(`${flagSeverityIcon(flag.severity)} **[${flag.severity.toUpperCase()}]** ` +
                `\`${flag.flag_id}\`  `);
 ...
 */
function emitFlagReport(run_id, scenario_id, flags) {
    const lines = [];
    ...
    if (flags.length === 0) {
        ...
        for (const f of flags) {
            lines.push(`- ${flagSeverityIcon(f.severity)} \`${f.flag_id}\` [${f.severity}] — ${f.message}`);
        }
    }
    return lines.join('\n');
}
