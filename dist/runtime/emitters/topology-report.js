...
exports.emitTopologyReport = emitTopologyReport;
exports.renderTopologyReportMarkdown = renderTopologyReportMarkdown;
/**
...
function emitTopologyReport(result, zones = []) {
    ...
    const zoneEntries = zones.map(z => ({
        ...
        convergence_enabled: z.convergence_enabled ?? false,
        isolation_boundary: z.isolation_boundary ?? false,
        upstream_zone_ref: z.upstream_zone_ref ?? null,
        downstream_zone_ref: z.downstream_zone_ref ?? null,
    ...
    return {
        ...
        violations: allViolations.map(v => ({ rule: v.rule, severity: v.severity ?? 'error', message: v.message })),
        warnings: allWarnings.map(w => ({ rule: w.rule, severity: w.severity ?? 'warning', message: w.message })),
 ...
 */
function renderTopologyReportMarkdown(report) {
    const lines = [];
    ...
    if (report.blocking_errors.length > 0) {
        ...
        for (const err of report.blocking_errors)
            lines.push(`- ${err}`);
        lines.push('');
    }
    ...
    if (report.defaults_applied.length === 0) {
        ...
        for (const d of report.defaults_applied)
            lines.push(`- ${d}`);
    }
    return lines.join('\n');
}
//# sourceMappingURL=topology-report.js.map