...
exports.validatePickupGeometryBounds = validatePickupGeometryBounds;
exports.validateRadiator3ABounds = validateRadiator3ABounds;
exports.validateConvergenceControl = validateConvergenceControl;
exports.validateResistanceChain = validateResistanceChain;
exports.validateExtension3ABounds = validateExtension3ABounds;
 ...
 * §8.3, §13.2.
 */
function validatePickupGeometryBounds(entry) {
    const v = [];
 ...
 * §9.3, §13.3.
 */
function validateRadiator3ABounds(rad, tSinkResolvable) {
    const v = [];
 ...
 * §5.4, §13.4.
 */
function validateConvergenceControl(cc) {
    const v = [];
    ...
    if (cc.runaway_multiplier < constants_1.CONVERGENCE_RUNAWAY_MULTIPLIER_MIN) {
        v.push({ rule: '3A-spec §13.4', severity: 'error', field: 'convergence_control.runaway_multiplier', value: cc.runaway_multiplier,
            message: `runaway_multiplier must be >= ${constants_1.CONVERGENCE_RUNAWAY_MULTIPLIER_MIN}. Got ${cc.runaway_multiplier}. §5.4, §13.4` });
    }
    return v;
}
 ...
 * §6.5, §13.5.
 */
function validateResistanceChain(zone, pickupGeometryResolved) {
    const v = [];
    const w = [];
 ...
 */
function validateExtension3ABounds(params) {
    const allViolations = [];
...
//# sourceMappingURL=extension-3a-bounds.js.map