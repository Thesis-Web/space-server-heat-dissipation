...
exports.validateStageCompleteness = validateStageCompleteness;
exports.validateStageCoefficientBounds = validateStageCoefficientBounds;
exports.validateStageProvenance = validateStageProvenance;
exports.validateBaselineIsolation = validateBaselineIsolation;
exports.validateDerivedProfileIntegrity = validateDerivedProfileIntegrity;
exports.validateExtension2 = validateExtension2;
// ---------------------------------------------------------------------------
 ...
 */
function validateStageCompleteness(stages) {
    const results = [];
    for (const stage of stages) {
        ...
        if (stage.source_spectral_profile_ref === null ||
 ...
 */
function validateStageCoefficientBounds(stage) {
    const results = [];
    const bounds = [
 ...
 */
function validateStageProvenance(stage, strictResearchEnforcement) {
    const results = [];
    if (!HIGH_RISK_PROVENANCE.has(stage.provenance_class))
        return results;
    // Stage has risky provenance — check if it has output-driving fields with non-zero effect
    const hasNonZeroOutput = stage.capture_efficiency_fraction > 0 ||
        stage.band_match_score > 0 ||
        stage.geometry_coupling_score > 0 ||
        stage.mediator_transfer_score > 0;
    if (hasNonZeroOutput && stage.enabled) {
        const severity = strictResearchEnforcement ? 'blocking' : 'caution';
        results.push({
            severity,
 ...
 */
function validateBaselineIsolation(modelExtension2Mode, _baselineResultPresent, exploratoryResultPresent) {
    const results = [];
    ...
    if ((modelExtension2Mode === 'exploratory_compare' || modelExtension2Mode === 'exploratory_only') &&
 ...
 */
function validateDerivedProfileIntegrity(derivedProfiles) {
    const results = [];
 ...
 * Spec §15 complete.
 */
function validateExtension2(stages, derivedProfiles, modelExtension2Mode, strictResearchEnforcement, baselineResultPresent, exploratoryResultPresent) {
    const allResults = [];
    // §15.1 completeness
    ...
    allResults.push(...validateBaselineIsolation(modelExtension2Mode, baselineResultPresent, exploratoryResultPresent));
    // §15.7 derived profile integrity
    allResults.push(...validateDerivedProfileIntegrity(derivedProfiles));
...
//# sourceMappingURL=extension-2-bounds.js.map