...
function buildGraph(zones) {
    ...
    for (const z of zones) {
        ...
        if (!adj.has(z.zone_id))
            adj.set(z.zone_id, new Set());
        if (!inDegree.has(z.zone_id))
            inDegree.set(z.zone_id, 0);
    }
 ...
 */
function kahnTopologicalSort(adj, inDegree, nodeSet) {
 ...
 * §13.1 blocking rule.
 */
function validateIsolationBoundaries(zones) {
    const violations = [];
    for (const z of zones) {
        if (z.isolation_boundary) {
            const r = z.bridge_resistance_k_per_w;
...
function buildWarnings(zones) {
    ...
    for (const z of zones) {
        ...
        if (z.convergence_enabled) {
            ...
            const hasConvergenceNeighbor = zones.some(other => neighbors.includes(other.zone_id) && other.convergence_enabled);
 ...
 */
function validateTopology(zones, policy = 'blocking') {
    const violations = [];
    const warnings = [...buildWarnings(zones)];
